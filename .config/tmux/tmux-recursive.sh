...
if [[ -d $selected ]]; then
    if [[ $key == 'ctrl-space' ]]; then
        # If Ctrl-Space was pressed, cd into the directory in the current window
    ...
    if [[ $key == 'ctrl-space' ]]; then
        # If Ctrl-Space was pressed, open the file in the current window
        tmux send-keys "cd $parent_dir && nvim $(basename "$selected")" C-m
    else
        # If Enter was pressed, open a new tmux window with the file name, cd into the parent directory, and open the file in nvim
